...
export default async function ComparePage({searchParams}:{searchParams:Promise<{ids?:string}>}){
  ...
  if(ids.length<2) return <main className="mx-auto max-w-6xl px-6 py-20"><p className="text-xs uppercase tracking-[.2em] text-silver">Office Compare</p><h1 className="mt-2 font-display text-3xl">비교할 오피스를 선택해 주세요.</h1><p className="mt-3 text-sm leading-6 text-silver">건물 검색에서 2~4개 후보를 선택하면 면적·임대료·교통·주차·Prime Score를 한 화면에서 비교할 수 있습니다.</p><Link className="mt-6 inline-block bg-navy px-5 py-2.5 text-sm text-white" href="/buildings">오피스 찾기</Link></main>;
  ...
  return <main className="min-h-screen bg-fog">
    ...
    <div className="mx-auto max-w-6xl px-6 py-10"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><p className="text-sm"><b>{buildings.length}개</b> 건물을 비교 중입니다.</p><div className="flex gap-2"><Link href="/buildings" className="border border-silver/40 bg-white px-4 py-2 text-xs">후보 다시 선택</Link><Link href="/advisory" className="bg-navy px-4 py-2 text-xs text-white">이 조건으로 상담</Link></div></div>
      <div className="overflow-x-auto border border-silver/30 bg-white"><table className="w-full min-w-[850px] text-sm"><thead><tr><th className="bg-fog p-3"></th>{buildings.map((b:any)=><th key={b.id} className="border-l border-silver/20 p-4 text-left"><Link className="font-display text-lg hover:underline" href={`/buildings/${b.slug}`}>{b.name}</Link><p className="mt-1 text-[11px] font-normal text-silver">상세보기 →</p></th>)}</tr></thead><tbody>
        {row("등급",b=>b.building_grade??"-")}{row("준공",b=>b.completion_year?`${b.completion_year}년`:"-")}{row("주소",b=>b.road_address??"-")}{row("규모",b=>`지상 ${b.above_ground_floors??"-"}층 / 지하 ${b.basement_floors??"-"}층`)}{row("주차",b=>{const p=Array.isArray(b.building_parking)?b.building_parking[0]:b.building_parking;return p?.total_spaces?`${p.total_spaces}대`:"-"})}{row("교통",b=>{const ts=Array.isArray(b.building_transportation)?b.building_transportation:[];const t=ts[0];return t?`${t.station_name??""}${t.walk_minutes?` · 도보 ${t.walk_minutes}분`:""}`:"-"})}{row("Prime Score",b=>{const s=Array.isArray(b.building_scores)?b.building_scores[0]:b.building_scores;return s?.status==='PUBLISHED'?s.total_score:"평가 준비중"})}{row("현재 공개 공실",b=>`${active(b).length}건`)}{row("대표 전용면적",b=>{const l=representative(b);return l?.exclusive_area_py?`${Number(l.exclusive_area_py).toLocaleString()}평`:"-"})}{row("평당 임대료",b=>money(representative(b)?.rent_per_py))}{row("평당 관리비",b=>money(representative(b)?.maintenance_per_py))}{row("NOC",b=>money(representative(b)?.noc_per_py))}{row("입주 가능",b=>representative(b)?.move_in_text??"협의")}
      </tbody></table></div>
