...
import BuildingFactsEnrichmentButton from "@/components/admin/BuildingFactsEnrichmentButton";
...
export default async function ImportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: stagingBuildingsData } = latestRealRun ? await supabase.from("staging_buildings").select("id,raw_building_name,primary_source_page,match_status,match_score,match_reason,matched_building_id,extracted_data").eq("parsing_run_id",latestRealRun.id).order("primary_source_page") : { data: [] as any[] };
  ...
  // CBRE v1.14 added post-approval building-facts enrichment. Existing CBRE-v1.8.x staging
  // remains semantically compatible and must not be invalidated just because the enrichment engine advanced.
  const cbreStoredVersionCompatible = doc.parser_type === "CBRE" && typeof doc.parser_version === "string" && /^CBRE-v1\.8(?:\.|$)/.test(doc.parser_version);
  const stagingIsCurrent = stagingBuildings.length === 0 || !expectedSemanticVersion || doc.parser_version === expectedSemanticVersion || cbreStoredVersionCompatible;

  const warningRows: Array<{ id:string; scope:"BUILDING"|"LISTING"; buildingName:string; floor:string|null; page:number|null; reason:string }> = [];
  for (const b of stagingBuildings as any[]) {
    const ws = Array.isArray(b.extracted_data?._warnings) ? b.extracted_data._warnings : [];
    ws.forEach((reason:any, idx:number) => warningRows.push({ id:`b-${b.id}-${idx}`, scope:"BUILDING", buildingName:b.raw_building_name ?? "이름 없음", floor:null, page:b.primary_source_page ?? null, reason:String(reason) }));
  }
  for (const l of stagingListings as any[]) {
  ...
  for (const b of stagingBuildings as any[]) {
    const infos = Array.isArray(b.extracted_data?._infos) ? b.extracted_data._infos : [];
    infos.forEach((reason:any, idx:number) => infoRows.push({ id:`bi-${b.id}-${idx}`, scope:"BUILDING", buildingName:b.raw_building_name ?? "이름 없음", floor:null, page:b.primary_source_page ?? null, reason:String(reason) }));
  }
  for (const l of stagingListings as any[]) {
    const infos = Array.isArray(l.extracted_data?._infos) ? l.extracted_data._infos : [];
    infos.forEach((reason:any, idx:number) => infoRows.push({ id:`li-${l.id}-${idx}`, scope:"LISTING", buildingName:buildingNameById.get(l.staging_building_id) ?? "이름 없음", floor:l.floor ?? null, page:l.source_page ?? null, reason:String(reason) }));
  }
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="mb-5 grid gap-3 md:grid-cols-5">
        ...
        <a href="#warning-review" className={`block border p-4 transition-colors ${(doc.warning_count ?? 0)>0?"border-amber-300 bg-amber-50 hover:bg-amber-100":"border-silver/25 bg-white"}`}><p className="text-xs text-silver">검토 경고</p><p className="mt-1 text-lg font-medium tabular-nums">{doc.warning_count ?? 0}</p>{(doc.warning_count ?? 0)>0&&<p className="mt-1 text-[11px] font-medium text-amber-800">클릭해서 경고 목록 보기 →</p>}</a>
        <a href="#information-review" className={`block border p-4 transition-colors ${infoRows.length>0?"border-sky-200 bg-sky-50 hover:bg-sky-100":"border-silver/25 bg-white"}`}><p className="text-xs text-silver">참고 정보</p><p className="mt-1 text-lg font-medium tabular-nums">{infoRows.length}</p>{infoRows.length>0&&<p className="mt-1 text-[11px] font-medium text-sky-800">오류 아닌 참고 항목 보기 →</p>}</a>
      </div>
      ...
      {doc.parser_type && ["NAI","CBRE","CW"].includes(doc.parser_type) && <section className="mb-5 border border-silver/25 bg-white p-5"><div className="mb-4"><p className="text-xs uppercase tracking-[.16em] text-silver">Step 2–4</p><h2 className="mt-1 font-medium">구조화 · 건물 매칭 · 변경 검수 · 승인</h2><p className="mt-1 text-xs leading-5 text-silver kr-text">원문에서 추출한 후보는 임시 검수영역에만 저장됩니다. 마지막 ‘최종 승인·반영’을 누르기 전에는 실제 건물·공실 DB를 변경하지 않습니다.</p>{expectedSemanticVersion && <p className="mt-2 text-[11px] text-silver">현재 구조화 엔진 <b>{expectedSemanticVersion}</b>{doc.parser_version ? ` · 저장된 구조화 결과 ${doc.parser_version}${cbreStoredVersionCompatible ? " · 호환됨" : ""}` : " · 아직 구조화 결과 없음"}</p>}</div>
        ...
        {stagingListings.length > 0 && <details className="mt-5 border-t border-silver/20 pt-4" open={doc.status !== "APPROVED"}><summary className="cursor-pointer text-sm font-medium">최종 승인 전 공실 데이터 확인 ({stagingListings.length}건)</summary><p className="mt-2 text-xs leading-5 text-silver kr-text">층·면적·평당 임대료·관리비·입주시기를 원본 PDF와 대조해 주세요. 특히 ‘충돌’, ‘업데이트’, 경고가 있는 자료는 반드시 확인한 뒤 승인하는 것을 권장합니다.</p><div className="mt-3 max-h-[620px] overflow-auto border border-silver/20"><table className="min-w-[1320px] w-full text-left text-xs"><thead className="sticky top-0 bg-fog"><tr><th className="px-3 py-2">건물</th><th className="px-3 py-2">층</th><th className="px-3 py-2">임대면적(평)</th><th className="px-3 py-2">전용면적(평)</th><th className="px-3 py-2">임대료/평</th><th className="px-3 py-2">관리비/평</th><th className="px-3 py-2">총 보증금</th><th className="px-3 py-2">총 월세</th><th className="px-3 py-2">총 관리비</th><th className="px-3 py-2">입주</th><th className="px-3 py-2">판정</th><th className="px-3 py-2">출처</th></tr></thead><tbody>{stagingListings.slice(0,200).map((row:any)=>{const d=row.extracted_data; const warnings=Array.isArray(d?._warnings)?d._warnings:[]; return <tr key={row.id} className={`border-t border-silver/15 ${row.change_type==="CONFLICT"?"bg-orange-50":row.change_type==="UPDATED"?"bg-amber-50/50":"bg-white"}`}><td className="px-3 py-2 font-medium">{buildingNameById.get(row.staging_building_id) ?? "-"}</td><td className="px-3 py-2">{row.floor ?? String(extractedValue(d,"floor_raw") ?? "-")}</td><td className="px-3 py-2 tabular-nums">{formatNumber(extractedValue(d,"gross_area_py"))}</td><td className="px-3 py-2 tabular-nums">{formatNumber(extractedValue(d,"exclusive_area_py"))}</td><td className="px-3 py-2 tabular-nums">{formatNumber(extractedValue(d,"rent_per_py"))}</td><td className="px-3 py-2 tabular-nums">{formatNumber(extractedValue(d,"maintenance_per_py"))}</td><td className="px-3 py-2 tabular-nums">{formatNumber(extractedValue(d,"deposit_total_won"))}</td><td className="px-3 py-2 tabular-nums">{formatNumber(extractedValue(d,"monthly_rent_total_won"))}</td><td className="px-3 py-2 tabular-nums">{formatNumber(extractedValue(d,"management_fee_total_won"))}</td><td className="px-3 py-2">{String(extractedValue(d,"move_in_text") ?? "-")}</td><td className="px-3 py-2"><span>{row.change_type ?? "UNCLASSIFIED"}</span>{warnings.length>0&&<div className="mt-1 text-[10px] text-amber-700">경고 {warnings.length}</div>}</td><td className="px-3 py-2 tabular-nums">p.{row.source_page ?? "-"}</td></tr>})}</tbody></table></div>{stagingListings.length>200&&<p className="mt-2 text-xs text-silver">화면에는 앞 200건을 표시합니다. 전체 {stagingListings.length}건은 승인 로직에 모두 포함됩니다.</p>}</details>}
        {stagingBuildings.length > 0 && <details className="mt-5 border-t border-silver/20 pt-4" open={doc.status !== "APPROVED"}><summary className="cursor-pointer text-sm font-medium">추출된 건물 후보 {stagingBuildings.length}개 보기</summary><div className="mt-3 max-h-[520px] space-y-2 overflow-y-auto">{stagingBuildings.map((b:any)=><div key={b.id} className="border border-silver/20 p-3 text-sm"><div className="flex items-center justify-between gap-3"><span className="font-medium">{b.raw_building_name || "이름 없음"}</span><span className="text-xs text-silver">p.{b.primary_source_page ?? "-"} · 공실 {listingCountByBuilding.get(b.id) ?? 0}</span></div><div className="mt-1 flex flex-wrap gap-x-3 text-xs text-silver"><span>매칭 {b.match_status ?? "미처리"}</span>{typeof b.match_score === "number" && <span>신뢰도 {Math.round(b.match_score*100)}%</span>}{b.match_reason && <span>{b.match_reason}</span>}</div></div>)}</div></details>}
      </section>}

      {doc.parser_type === "CBRE" && doc.status === "APPROVED" && <div className="mb-5"><BuildingFactsEnrichmentButton sourceDocumentId={doc.id} parserType={doc.parser_type} /></div>}
