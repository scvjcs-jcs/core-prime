    ...
    <main id="main-content">
      <section className="relative overflow-hidden bg-navy text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-6 py-14 md:py-20 lg:grid-cols-[1.03fr_.97fr] lg:items-stretch">
          <div className="flex flex-col justify-center py-2 lg:py-8">
            ...
            <h1 className="max-w-3xl font-display text-[42px] leading-[1.08] md:text-6xl kr-text">좋은 오피스를 찾는 시간을 줄이고,<br className="hidden md:block"/> 결정의 정확도를 높입니다.</h1>
            <p className="mt-7 max-w-2xl text-[15px] leading-7 text-silver md:text-base kr-text">공개 매물만 나열하지 않습니다. 건물 정보와 현재 공실을 검수하고, 기업의 면적·예산·입주 일정에 맞는 후보를 데이터와 자문으로 좁혀드립니다.</p>
            ...
            <div className="mt-9 grid max-w-xl grid-cols-3 border-t border-white/15 pt-5">
              <div><p className="text-[10px] uppercase tracking-[.14em] text-silver">Published</p><p className="mt-1 font-display text-2xl tabular-nums">{publishedBuildings ?? 0}</p><p className="mt-1 text-[11px] text-silver">공개 빌딩</p></div>
          ...
          <div className="relative min-h-[480px] overflow-hidden border border-white/15 bg-white/5 lg:min-h-[560px]">
            {heroImage ? <img src={heroImage} alt={heroBuilding?.name ?? "서울 프라임 오피스"} className="absolute inset-0 h-full w-full object-cover"/> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_20%,rgba(255,255,255,.12),transparent_30%),linear-gradient(145deg,#1b2d4b,#0d1b30_62%,#182843)]"/>}
            <div className="absolute inset-0 bg-gradient-to-t from-navy via-navy/25 to-transparent"/>
            {heroBuilding && <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8"><p className="text-[10px] uppercase tracking-[.18em] text-silver">Selected Building</p><div className="mt-2 flex items-end justify-between gap-4"><div><p className="font-display text-2xl">{heroBuilding.name}</p><p className="mt-1 text-xs text-silver">{heroBuilding.districts?.name ?? "서울"} · {heroBuilding.building_grade ?? "등급 확인중"}</p></div><Link href={`/buildings/${heroBuilding.slug}`} className="border border-white/30 px-3 py-2 text-xs hover:bg-white hover:text-navy">상세보기 →</Link></div></div>}
            <div className="absolute right-5 top-5 border border-white/20 bg-navy/75 px-3 py-2 text-[11px] text-white backdrop-blur"><span className={`mr-2 inline-block h-1.5 w-1.5 rounded-full ${heroFreshness.tone === "stale" ? "bg-amber-300" : heroFreshness.tone === "muted" ? "bg-silver" : "bg-emerald-300"}`}/>{heroFreshness.label}</div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <div className="grid gap-10 lg:grid-cols-[.78fr_1.22fr] lg:items-start">
          <div className="lg:sticky lg:top-28"><p className="mb-3 text-[10px] uppercase tracking-[.2em] text-silver">Decision Framework</p><h2 className="font-display text-3xl leading-tight kr-text">매물 검색이 아니라<br/>의사결정 과정을 설계합니다.</h2><p className="mt-4 max-w-md text-sm leading-6 text-silver kr-text">후보를 많이 보여주는 것보다, 비교할 이유가 있는 후보만 남기는 것이 더 중요합니다.</p><Link href="/advisory" className="mt-6 inline-block text-sm font-medium text-navy hover:underline">기업 조건으로 추천받기 →</Link></div>
